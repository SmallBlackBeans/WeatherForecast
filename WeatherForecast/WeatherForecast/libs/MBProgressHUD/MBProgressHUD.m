...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.mode = MBProgressHUXCodeIndeterminate;
		self.labelText = nil;
...
- (void)updateIndicators {
	...
	if (mode == MBProgressHUXCodeIndeterminate) {
	...
	else if (mode == MBProgressHUXCodeDeterminateHorizontalBar) {
		// Update to bar determinate indicator
		[indicator removeFromSuperview];
		self.indicator = MB_AUTORELEASE([[MBBarProgressView alloc] init]);
		[self addSubview:indicator];
	}
	else if (mode == MBProgressHUXCodeDeterminate || mode == MBProgressHUXCodeAnnularDeterminate) {
		if (!isRoundIndicator) {
		...
		if (mode == MBProgressHUXCodeAnnularDeterminate) {
			[(MBRoundProgressView *)indicator setAnnular:YES];
		}
	} 
	else if (mode == MBProgressHUXCodeCustomView && customView != indicator) {
		// Update custom view indicator
		[indicator removeFromSuperview];
		self.indicator = customView;
		[self addSubview:indicator];
	} else if (mode == MBProgressHUXCodeText) {
		[indicator removeFromSuperview];
		self.indicator = nil;
	}
}
